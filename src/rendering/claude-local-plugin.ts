...
import { listDirectoryNames, listFilesRecursive, pathExists, readTextFileIfExists, writeTextFile } from "../infra/fs.js";
...
export const CLAUDE_LOCAL_PLUGIN_MARKETPLACE_ROOT = ".ticiou/.runtime/claude-plugin-marketplace";
...
export async function collectClaudeLocalProfilePluginFiles(user: string, config: TiciouConfig): Promise<ManagedFile[]> {
  const profileSkillRoot = join(resolveRuntimeProfilesDirectory(), "users", user, "skills");
  ...
  files.push({
    relativePath: posix.join(CLAUDE_LOCAL_PLUGIN_MARKETPLACE_ROOT, ".claude-plugin/marketplace.json"),
  ...
  files.push({
    relativePath: posix.join(CLAUDE_LOCAL_PLUGIN_MARKETPLACE_ROOT, "plugins", pluginName, ".claude-plugin/plugin.json"),
  ...
  for (const skillName of skillNames) {
    ...
    for (const resourceFile of await listFilesRecursive(skillRoot)) {
      ...
      const content =
        resourceFile === "SKILL.md" ? normalizeSkillFrontmatterName(rawContent, pluginSkillName) : rawContent;
...
export async function syncClaudeLocalProfilePluginSettings(
  ...
  const marketplacePath = join(targetRoot, ...CLAUDE_LOCAL_PLUGIN_MARKETPLACE_ROOT.split("/"));

  removeTiciouPluginSettings(settings);

  if (user !== undefined) {
    const pluginName = userPluginName({ prefix: config.render.prefix, user });
    setRecordValue(settings, "extraKnownMarketplaces", CLAUDE_LOCAL_PROFILE_MARKETPLACE, {
      source: {
        source: "directory",
        path: marketplacePath,
      },
    });
    setRecordValue(settings, "enabledPlugins", `${pluginName}@${CLAUDE_LOCAL_PROFILE_MARKETPLACE}`, true);
  }
...
export async function installClaudeLocalProfilePlugin(options: ClaudePluginSyncOptions): Promise<void> {
  const runner = options.runner ?? defaultCommandRunner;
  const marketplaceRoot = join(options.targetRoot, ...CLAUDE_LOCAL_PLUGIN_MARKETPLACE_ROOT.split("/"));
  const pluginId = localProfilePluginId(options.config, options.user);

  await runner("claude", ["plugin", "marketplace", "add", marketplaceRoot, "--scope", "local"], {
...
export async function uninstallClaudeLocalProfilePlugin(options: ClaudePluginClearOptions): Promise<void> {
  const runner = options.runner ?? defaultCommandRunner;
  const pluginIds = await listInstalledTiciouPluginIds(options.targetRoot, runner);

  for (const pluginId of pluginIds) {
    await runner("claude", ["plugin", "uninstall", pluginId, "--scope", "local"], {
      cwd: options.targetRoot,
      env: process.env,
    });
  }
...
export async function getClaudeLocalProfilePluginStatus(
  ...
  const entry = parsed
    ...
    .find((value) => value.id === pluginId && value.scope === "local");
...
export function localProfilePluginId(config: TiciouConfig, user: string): string {
...
async function listInstalledTiciouPluginIds(targetRoot: string, runner: CommandRunner): Promise<string[]> {
  const result = await runner("claude", ["plugin", "list", "--json"], {
    cwd: targetRoot,
    env: process.env,
  });
  ...
  return parsed
    .filter((value): value is Record<string, unknown> => typeof value === "object" && value !== null)
    .map((value) => value.id)
    .filter((id): id is string => typeof id === "string" && id.endsWith(`@${CLAUDE_LOCAL_PROFILE_MARKETPLACE}`));
}
...
function setRecordValue(settings: Record<string, unknown>, key: string, recordKey: string, value: unknown): void {
